---
title: "Linear Mixed Effects Models - week 1"
...
LU_train$SUBJ <- str_replace_all(LU_train$SUBJ, pattern = "[[:punct:]]", replacement ="") 
token_train$SUBJ <- str_replace_all(token_train$SUBJ, pattern = "[[:punct:]]", replacement ="")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, pattern = "[[:punct:]]", replacement ="") 
...
#merging the data
train <- merge(token_train,LU_train, all = TRUE)
train <- merge(train,demo_train, all = TRUE)
...
#Renaming columns
colnames(train)[8] <- "nonVerbalIQ"
colnames(train)[9] <- "verbalIQ"
...
train_subset <- subset(train, VISIT == 1)
train_filter <- filter(train, VISIT == "1")

#Renaming columns
colnames(train_subset)[7] <- "ADOS1"
colnames(train_subset)[8] <- "nonverbalIQ1"
colnames(train_subset)[9] <- "verbalIQ1"
...
train <- merge(train,train_subset, by = "SUBJ")
...
SUBJ_list <- data.frame(unique(train$SUBJ)) #creating data frame with different subjects
SUBJ_list$ANO_SUBJ <- c(1:66) # adding column with numbers to anominize
colnames(SUBJ_list)[1] <- "SUBJ" #changeing column name to enable merge
train <- merge(train,SUBJ_list, by = "SUBJ") #Merging subset with final data set

#Alternative
#train <- as.integer(as.factor(train$SUBJ))

#Selecting relevant columns and arranging in correct order
train <- train %>% 
 as_data_frame %>%
  select(ANO_SUBJ, Gender, Age, Diagnosis, Ethnicity, VISIT,  ADOS1,  nonverbalIQ1, verbalIQ1, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd, types_MOT, types_CHI, tokens_MOT, tokens_CHI)

#Gender specification
train <- mutate(train, Gender = ifelse(Gender  %in% c("1"), "F", "M"))

#levels(train$Gender) <- c("F", "M")

#Diagonosis specification
train <- mutate(train, Diagnosis = ifelse(Diagnosis  %in% c("A"), "ASD", "TD"))
...
```{r}
write.csv(train, file = "autism_data.csv")
```
...
#1
MLU <- train %>% filter(CHI_MLU > 2.7) #creating dataframe with children with mean length utterances above 2.7 morphemes

#MLU <- train %>% group_by(ANO_SUBJ) %>% filter(mean(CHI_MLU) > 2.7) #creating dataframe with children with mean length utterances above 2.7 morphemes

unique(MLU$ANO_SUBJ) #listing the children

#2
MLU <- filter(train, CHI_MLU < 1.5 & VISIT == 1) #creating dataframe with children with mean length utterances below 1.5 morphemes at first visit

unique(MLU$ANO_SUBJ) #listing the children

#3
incomplete <- data.frame(table(train[1]))%>%filter(Freq != 6) #not 6 visits
incomplete <- train[which(is.na(train$CHI_MLU)), 1] #incomplete data sets
unique(incomplete)
...
```{r}
#1
VISIT_6 <- train %>% filter(VISIT == 6) 
arrange(VISIT_6, desc(tokens_CHI))%>% slice(1)

#2
VISIT_1 <- train %>% filter(VISIT == 1) 
arrange(VISIT_1, tokens_CHI) %>% slice(1)
```
...
```{r}
#1
train %>% select(Diagnosis, contains("MLU"), contains("tokens")) %>% filter(Diagnosis == "ASD")#filtering ASD diagnosis and selecting correct columns

#2
select_2 <- select(train, Diagnosis, Diagnosis) #R ignores ir
```
...
```{r}
#1
train <- train %>% group_by(ANO_SUBJ) %>% mutate(mean_spokenwords = mean(tokens_MOT+tokens_CHI))

#2½
#train <- train %>% group_by(ANO_SUBJ) %>% 
```
